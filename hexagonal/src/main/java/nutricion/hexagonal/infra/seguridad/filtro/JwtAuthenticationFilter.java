...
@Component
public class JwtAuthenticationFilter extends OncePerRequestFilter { // ANTES DE LLEGAR A CONTROLLADORES
    ...
    // 401,bakenc no puede autenticar request: token no valido, filtro no procesa
    // bien, cabecera no adeacuada
    @Override
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        // 2. Si el token es válido, obtener la autenticación y establecerla en el
        // contexto PROVIDER valida
        if (token != null && tokenProvider.validateToken(token)) {
          
            Authentication auth = tokenProvider.getAuthentication(token);
            SecurityContextHolder.getContext().setAuthentication(auth);
        }
        // 3. Continuar con el filtro con usuario autenticado en el sprin security
        // context
        chain.doFilter(request, response);
    }

    // antes de que spring llame al filtro, llama aqui y si da true, filtro JWT no
    // se ejecuta
    @Override
    protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
        String path = request.getServletPath();
        return path.equals("/api/auth/login") || path.equals("/") || path.equals("/auth.js") || path.equals("/login")
                || path.startsWith("/api/auth") || path.startsWith("/productos");
    }
